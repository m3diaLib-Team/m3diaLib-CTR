...
namespace m3d {
    ...
    void Thread::start(bool t_detached) {
        ...
        threadCreate(threadFunction, &m_data, m_stackSize, m_priority, -2, t_detached);
    }
    ...
    int Thread::join(long long unsigned int t_timeout) {
        if (m_started) {
            threadJoin(m_thread, t_timeout);
            return threadGetExitCode(m_thread);
        } else {
            return -1;
        }
    }
